...
title(" Midponit Method versus Analytical Solutions")
xlabel("Protein Plasma Concentration")
ylabel("Synthesis Rate")
h = legend ('Midpoint Method','Analytical Solution');
hold on;
grid
% To convert the sym into numerics to evaluate the error
aa = double(yy_values); 
error1=['Midpoint error: ' num2str(100*abs((aa(end)-y(end))/aa(end))) '%'];
